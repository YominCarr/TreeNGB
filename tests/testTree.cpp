...
TEST_F(TestTree, treeInitialization)
{
    ...
    ASSERT_NO_FATAL_FAILURE(
            ...
            for (int i = 0; i < MAXLEAVES; ++i) {
                ASSERT_EQ(0u, tree.leaves[i].key);
...
TEST_F(TestTree, particleInRootNode) {
    ...
    bool inNode = coordInsideNode(x, y, z, tree.leaves[0], BOX);
    ASSERT_TRUE(inNode);
}
...
TEST_F(TestTree, splitRootNode) {
    ...
    for (int i = 0, l = 0; i < 2; ++i) {
        for (int j = 0; j < 2; ++j) {
            for (int k = 0; k < 2; ++k, ++l) {
                double x, y, z;
                key2Coord(tree.leaves[l], &x, &y, &z, BOX);

                ASSERT_EQ(i * d[0], x) << " at l = " << l;
                ASSERT_EQ(j * d[1], y) << " at l = " << l;
                ASSERT_EQ(k * d[2], z) << " at l = " << l;

                ASSERT_EQ(1u, tree.leaves[l].level) << " at l = " << l;
